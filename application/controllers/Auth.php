...
class Auth extends MY_Controller {
	public function __construct() {
		...
		if($this->session->has_userdata('crc_access') && $this->session->userdata('crc_access') == true && $this->session->has_userdata('crc_admin_access') && $this->session->userdata('crc_admin_access') == true) {
			...
			if($this->input->cookie("crc_ud",true) != NULL) {
				$dataArr = $this->input->cookie("crc_ud",true);
	...
	public function login() {
		...
		$ip = getIpAddress();
		$cacheVar = 'crc_invalid_login_counter_'.$ip;
		...
		if(isset($_POST['submit'])) {
			...
			if(!$error) {
				...
				if(is_array($user) && count($user) > 0) {
					...
					if($user['role'] == "admin") {
						$dataArr['crc_admin_access'] = true;
					}
					$this->session->set_userdata($dataArr);
					$this->cache->delete($cacheVar);
					$this->input->set_cookie("crc_ud",$this->encryption->encrypt(base64_encode(json_encode($dataArr))),63072000);
					redirect(base_url(ADMIN_CONTROLLER),"location");
					exit();
				}
				else {
			...
			if(!$error) {
				if(empty($uoe)) {
					...
					if(is_array($user) && count($user) > 0) {
						...
						$cacheVar = "crc_pass_reset_".$resetCode;
						$dataArr = array();
						$dataArr['id'] = $id;
